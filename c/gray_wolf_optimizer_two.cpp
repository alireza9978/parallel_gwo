...
public:
  Solution solve(double lower_band[],
                 double upper_band[],
                 int dimension,
                 ...
                 bool enable_print = 0)
  {
    ...
    for (int iteration_number = 0; iteration_number < max_iteration; iteration_number++)
    {
      for (int i = 0; i < search_agents_number; i++)
      {
        ...
        fitnesses[i] = Eval::rastrigin(positions[i], dimension);
      }
  ...
  Solution solve_parallel(double lower_band[],
                          double upper_band[],
                          int dimension,
                          int search_agents_number = 10,
                          int max_iteration = 100,
...
#pragma omp parallel
    {
      ...
      {
        positions[j] = new double[dimension];
        for (int i = id; i < dimension; i += nthrds)
        {
          positions[j][i] = ((rand() / ((double)RAND_MAX)) * (upper_band[i] - lower_band[i]) + lower_band[i]);
        }
      }
...
#pragma omp parallel
      {
        ...
        id = omp_get_thread_num();
        nthrds = omp_get_num_threads();
        for (int i = id; i < search_agents_number; i += nthrds)
        {
          //Return back the search agents that go beyond the boundaries of the search space
          for (int j = 0; j < dimension; j++)
          {
...
#pragma omp parallel
  ...
  Solution solve_parallel_for(double lower_band[],
                              double upper_band[],
                              int dimension,
                              int search_agents_number = 10,
                              int max_iteration = 100,
...
#pragma omp parallel for collapse(2)
  ...
  Solution solve_parallel_both(double lower_band[],
                               double upper_band[],
                               int dimension,
                               int search_agents_number = 10,
                               int max_iteration = 100,
...
#pragma omp parallel
      {
        ...
        for (int i = id; i < search_agents_number; i += nthrds)
        {
          ...
          //Calculate objective function for each search agent
          fitnesses[i] = Eval::rastrigin_parallel(positions[i], dimension);
        }
      }
...
#pragma omp parallel
  ...
  Solution solve_parallel_for_both(double lower_band[],
                                   double upper_band[],
                                   int dimension,
                                   int search_agents_number = 10,
                                   int max_iteration = 100,
...
#pragma omp parallel for
      ...
      {
        //Calculate objective function for each search agent
        fitnesses[i] = Eval::rastrigin_parallel_for(positions[i], dimension);
      }
      double fitness;
      for (int i = 0; i < search_agents_number; i++)
      {
...
#pragma omp parallel for collapse(2)
  ...
  Solution solve_normal_parallel_objective(double lower_band[],
                                           double upper_band[],
                                           int dimension,
                                           int search_agents_number = 10,
                                           int max_iteration = 100,
  ...
  Solution solve_normal_parallel_for_objective(double lower_band[],
                                               double upper_band[],
                                               int dimension,
                                               int search_agents_number = 10,
                                               int max_iteration = 100,
